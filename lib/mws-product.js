...
exports = module.exports = function(options) {
  ...
  // Check params
  mpCur    = ('' + mpOpt) || mpDef;
  authInfo = {sellerId: null, accessKeyId: null, secretKey: null, mwsauthid: null };
  ...
  if(authOpt) {
    ...
    authInfo.mwsauthid   = authOpt.mwsauthid   || null;
  }
  ...
  mwsReqQryGen = function mwsReqQryGen(options) {
    ...
    var method     = (options && options.method) ? ('' + options.method) : 'POST',
        ...
        returnData = {
          ...
          "MWSAuthToken": authInfo.mwsauthid,
        ...
        if (typeof value === 'object') {
          for (var key in value) {
            if (value.hasOwnProperty(key)) {
              reqForm.query[name + '.' + (i+1) + '.' + key] = ('' + value[key]);
            }
          }
        } else {
          reqForm.query[name + '.' + (i+1)] = ('' + value);
        }
      }
  ...
  getMyFeesEstimate = function getMyFeesEstimate(options, callback) {
    
    var productData = (options && options.productData) ? options.productData : null;
    
    var reqForm = {query: {"Action": "GetMyFeesEstimate", "MarketplaceId": mpList[mpCur].id}};
    
    mwsReqParamFormatter(reqForm, "FeesEstimateRequestList.FeesEstimateRequest", productData);
    mwsReqProcessor(reqForm, 'getMyFeesEstimate', 'GetMyFeesEstimateResponse', 'GetMyFeesEstimateResult', 'mwsprod-015', callback);
  }
